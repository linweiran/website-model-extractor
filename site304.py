...
def gen_filename(type):
    num = COUNTERS[type]
    COUNTERS[type] += 1
    three_num = (num % 7) + (num // 7 * 10) + 33
    filename = "{}{}.{}".format(PATTERNS[type], three_num, type)
    return filename
	...
	for i in init:
		if init[i]<>"root":
			if type[i]=="Image":
				mark[i]=imgcounter
				imgcounter+=1
				filenamepad[i]=gen_filename("gif")
			if type[i]=="Stylesheet":
				mark[i]=sheetcounter
				sheetcounter+=1
				filenamepad[i]=gen_filename("css")
			if type[i]=="Font":
				mark[i]=fontcounter
				fontcounter+=1
				filenamepad[i]=gen_filename("woff")
			if type[i]=="Script":
				mark[i]=scriptcounter
				scriptcounter+=1
				filenamepad[i]=gen_filename("js")
			if type[i]=="Document":
				mark[i]=doccounter
				doccounter+=1
				filenamepad[i]=gen_filename("html")
	for i in init:
		if type[i]=="Image":
